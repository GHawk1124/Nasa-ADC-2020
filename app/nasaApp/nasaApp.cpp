...
*
* Code was derived from the above, modified by Garrett Comes for the Nasa ADC
*
...
class VulkanExample : public VulkanExampleBase
{
  public:
	...
	// Shading Options
	const char *items[3]      = {"None", "Altitude", "Slope"};
	int         current_index = 0;
	...
	{
		title       = "Nasa ADC Moon Visualization";
	...
	void loadAssets()
	{
		...
		textures.skySphere.loadFromFile(getAssetPath() + "textures/space.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
		// Terrain textures are stored in a texture array with layers corresponding to terrain height
		textures.terrainArray.loadFromFile(getAssetPath() + "textures/lunar.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);

		// Height data is stored in a one-channel texture
		textures.heightMap.loadFromFile(getAssetPath() + "textures/terrain_heightmap.ktx", VK_FORMAT_R16_UNORM, vulkanDevice, queue);
...
#define UV_SCALE 1.0f
		...
		HeightMap heightMap(getAssetPath() + "textures/terrain_heightmap.ktx", PATCH_SIZE);
	...
	{
		if (overlay->header("Tesselation Settings"))
		{
		...
		if (overlay->header("Angles"))
		{
			overlay->text("Astronaut Position: (%f, %f, %f)", camera.position.x, camera.position.y, camera.position.z);
			overlay->text("Earth Position: (%f, %f, %f)", posEarth.x, posEarth.y, posEarth.z);
			overlay->text("Elevation Angle to Earth: %f", calcElevationCartesian(camera.position, posEarth));
			overlay->text("Zenith Angle to Earth: %f", calcZenithCartesian(camera.position, posEarth));
			overlay->text("Azimuth Angle to Earth: %f", calcAzimuthCartesian(camera.position, posEarth));
		}
		if (overlay->header("Pathfinding Controls"))
		{
			if (overlay->sliderInt("Max slope angle", &max_slope, 5, 40))
			{
				updateUniformBuffers();
			}
			if (overlay->sliderInt("Min slope angle", &min_slope, 0, 35))
			{
				updateUniformBuffers();
			}
			if (overlay->sliderInt("Max checkpoint distance", &distance, 0, 50))
			{
				updateUniformBuffers();
			}
			if (overlay->button("Recompute path"))
			{
				updateUniformBuffers();
			}
		}
		if (overlay->header("Recolor options"))
		{
			...
			if (ImGui::BeginCombo("Color type", combo_label, 0))
			{
				...
				{
					if (ImGui::Selectable(items[n], current_index == n))
					{
						current_index = n;
					}
